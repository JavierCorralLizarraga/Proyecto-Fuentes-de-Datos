...
grep -nP '(?<=place_id=")\d{5}(?=">)' $precios | awk -F ":" '!_[$2]++' | cut  -d : -f 1  > aux # return archivo con numeros de linea del archivo precios en la posicion del id y lidiamos con ids repetidos
line1="$(awk 'NR==1' aux)" 
line2="$(awk 'NR==2' aux)"  
...
do  
    ...
    id="$(grep -oP '(?<=place_id=")\d{5}(?=">)' BloqueEstacion)" # matcheamos el id del bloque
